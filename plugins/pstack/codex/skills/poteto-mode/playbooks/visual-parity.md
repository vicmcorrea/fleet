...
4. Verify each component against its baseline by image diff on the matching controlled surface. A nonzero diff is a fail. Iterate within the current task until zero; use goals or heartbeats only when the user authorized a long-running lifecycle.
