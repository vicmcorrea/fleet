...
8. **Watch the drain, do not drive it.** When the shipping request authorizes continued monitoring, arm the watcher in queued mode over the verified run and use a thread heartbeat until COMPLETE at the ceiling. Rearm after acted-on verdicts. ADVANCE is progress, not termination. Report each merge and the new ceiling. If the queue stalls, diagnose before mutating.
